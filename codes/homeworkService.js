...
function addHomework () {
  ...
  fetch("https://api.todoist.com/rest/v1/tasks", requestOptions)
    ...
    .then(result => referenceElement.insertAdjacentHTML('afterend', `<div class="goodWarning"><ion-icon name="checkmark-circle"></ion-icon>Adicionado com sucesso!</div>`))
    .catch(error => referenceElement.insertAdjacentHTML('afterend', `<div class="badWarning"><ion-icon name="alert"></ion-icon><p style="margin: 0px !important;">Eita. Deu erro. ${error}</p></div>`));
}
